...
@Singleton
@Named
public class Super6Lottery extends Lottery implements Serializable {

    private static final long serialVersionUID = -858035592864742081L;

    public static final String IDENTIFIER = "super6";
    ...
    @EJB
    Super6WinCalculator super6WinCalculator;


    public Super6Lottery() {
    }
    ...
    public ZonedDateTime getNextDrawing(final ZonedDateTime startDate, final DrawingType drawingType) {
        ...
        if ((weekDay < 3) || (weekDay == 3 && currentTime.isBefore(LocalTime.of(17,45)))) {
            drawingDate = baseTimstamp.plusDays(3 - weekDay).withHour(18).withMinute(0).withSecond(0).withNano(0);
            ...
            drawingDate = baseTimstamp.plusDays(10 - weekDay).withHour(18).withMinute(0).withSecond(0).withNano(0);
    ...
    public Ticket createNewEmbeddedTicket(MainTicket parentTicket, String newAdditionalLotteries) {
        throw new UnsupportedOperationException("Super6 cannot have embedded tickets");
    }

    @Override
    public WinCalculator getWinCalculator() {
        return super6WinCalculator;
    }
