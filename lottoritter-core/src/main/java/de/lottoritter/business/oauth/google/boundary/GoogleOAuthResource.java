 ...
 */
package de.lottoritter.business.oauth.google.boundary;
...
import de.lottoritter.business.oauth.google.control.GoogleOAuthController;
...
@LocalBean
@Path("google")
public class GoogleOAuthResource extends AbstractRestResource implements Serializable {

    private static final long serialVersionUID = 8419610066686060345L;

    private static final Logger logger = Logger.getLogger(GoogleOAuthResource.class.getName());
    ...
    public Response receiveAuthenticationCallback() {
        googleOAuthController.checkAndHandleAuthenticationError(httpServletRequest, httpServletResponse, OAuthProvider.GOOGLE);
        OAuthToken tokens = getTokenFromAuthenticationCode(httpServletRequest);
        OAuthData oAuthData = new OAuthData(OAuthProvider.GOOGLE);
        try {
            final OAuthUserProfileDTO userProfile = googleOAuthController.getUserProfile(tokens,
            ...
            googleOAuthController.handleRegistrationOrLogin(httpServletRequest, httpServletResponse, player, userProfile, oAuthData);
    ...
    private OAuthToken getTokenFromAuthenticationCode(HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            OAuthClientRequest request = OAuthClientRequest
                    ...
                    .setClientId(googleOAuthController.getClientId())
                    .setClientSecret(googleOAuthController.getClientSecret())
                    .setRedirectURI(configServerBaseUri.get() + "/api/google/oauth")
                    .setCode(code)
                    .buildBodyMessage();
            ...
            tokens.setExpires(currentTime + oAuthJSONAccessTokenResponse.getExpiresIn());
            tokens.setRefreshToken(oAuthJSONAccessTokenResponse.getRefreshToken());
            tokens.setIdToken(oAuthJSONAccessTokenResponse.getParam("id_token"));
